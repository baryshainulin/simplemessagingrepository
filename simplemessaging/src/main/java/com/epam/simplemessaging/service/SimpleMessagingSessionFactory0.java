...
public class SimpleMessagingSessionFactory0 {
  ...
  private static final Logger theLogger = Logger.getLogger(SimpleMessagingSessionFactory0.class);

  private static SimpleMessagingSessionFactory0 theInstance;
  
  // this field contains an implementation of blocking queue
  private SimpleBlockingQueue<Message> blockingQueue;

  private SimpleMessagingSessionFactory0(SimpleBlockingQueue<Message> queueImpl) {
    super();
    this.blockingQueue = queueImpl;
  }

  @Bean(name = "messaging")
  public static SimpleMessagingSessionFactory0 getInstance() {
    if (theInstance == null) {
      SimpleBlockingQueue<Message> queueImpl = getQueueImpl();
      theInstance = new SimpleMessagingSessionFactory0(queueImpl);
    }
    return theInstance;
  }
  ...
  private static SimpleBlockingQueue<Message> getQueueImpl() {
    theLogger.info("An instance of SimpleMessagingSessionFactory0 is to be created.");
    
    InputStream propertiesResource = SimpleMessagingSessionFactory0.class.getClassLoader().getResourceAsStream(APPLICATION_PROPERTIES_LOCATION);
